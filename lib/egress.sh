...
egress_start() {
  ...
  /usr/bin/perl "$script" "$portfile" "$(egress_allow_file)" >/dev/null 2>&1 &
  EGRESS_PROXY_PID=$!
  local port="" waited=0
  ...
  if [ -z "$port" ]; then
    ...
    kill "$EGRESS_PROXY_PID" 2>/dev/null || true; EGRESS_PROXY_PID=""
    return 1
  fi
  printf '%s\n' "$port"
}
...
egress_run() {
  ...
  local port; port="$(egress_start)" || store_exec -- "$@"   # start failed (warned) → honest fallback (execs)
  export HTTPS_PROXY="http://127.0.0.1:$port" HTTP_PROXY="http://127.0.0.1:$port" ALL_PROXY="http://127.0.0.1:$port"
  ...
  agsec_note "egress allowlist active (proxy 127.0.0.1:$port; $(egress_rule_count) rule(s)) — non-allowlisted hosts refused"
  trap 'kill "${EGRESS_PROXY_PID:-0}" 2>/dev/null || true' EXIT INT TERM
  store_exec_managed -- "$@"
  local rc=$?
  kill "${EGRESS_PROXY_PID:-0}" 2>/dev/null || true
  exit "$rc"
}
